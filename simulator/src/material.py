...
def MaterialLoadResponse(parent):
    MaterialLoad = Response(parent, parent.adapter, parent.material_load, 'material', 'UNLOADED', 'HAS_MATERIAL', parent.material_state, rel = True, simulate = True)
    MaterialLoad.create_statemachine()
    MaterialLoad.superstate.start()
    return MaterialLoad
...
def MaterialUnloadResponse(parent):
    MaterialUnload = Response(parent, parent.adapter, parent.material_unload, 'material', 'LOADED', 'HAS_MATERIAL', parent.material_state, rel = True, simulate = True)
    MaterialUnload.create_statemachine()
    MaterialUnload.superstate.start()
    return MaterialUnload
