                ...
                try:
                    ...
                    if 'Availability' in event.tag or 'Execution' in event.tag or 'ControllerMode' in event.tag:
                        ...
                        if ('AssetChanged' in event.tag or 'BindingState' in event.tag or self.binding_state_material.value() == "COMMITTED") and event.text.lower() != 'unavailable':
                            ...
                            if 'AssetChanged' in event.tag and self.binding_state_material.value() != "COMMITTED":
                                ...
                                #print "2_bind"
                                thread3= Thread(target = self.start_pull_asset,args=(addr,"/asset/",self.master_uuid, [event,source,component,x.attrib['uuid']]))
                                thread3.start()

                            elif self.binding_state_material.value() == "COMMITTED":
                                thread4= Thread(target = self.start_pull_asset,args=(addr,"/asset/",self.master_uuid, [event,source,component,x.attrib['uuid']]))
                                thread4.start()
...
def from_long_pull_asset(self,chunk, stream_root = None):
    ...
    print 'AFTER processing'+root[0].attrib['creationTime']+datetime.datetime.now().isoformat()
