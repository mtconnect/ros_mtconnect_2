...
def MaterialLoadResponse(parent):
    MaterialLoad = Response(parent, 'adapter', parent.material_load, 'material', 'LOADED', '', parent.material_state, rel = True, simulate = True)
    MaterialLoad.create_statemachine()
    MaterialLoad.superstate.start()
    return MaterialLoad
...
def MaterialUnloadResponse(parent):
    MaterialUnload = Response(parent, 'adapter', parent.material_unload, 'material', 'UNLOADED', '', parent.material_state, rel = True, simulate = True)
    MaterialUnload.create_statemachine()
    MaterialUnload.superstate.start()
    return MaterialUnload
